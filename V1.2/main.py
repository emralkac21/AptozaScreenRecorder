...
from TimelineAssetsModule import AssetOverlay, AssetEditorDialog, build_asset_vf
...
# ──────────────────────────────────────────────────────────────
def query_webcam_resolutions(cam_idx: int) -> list:
    ...
    cap.release()
    return supported if supported else [(1280, 720)]


class WebcamVideoStream:

    def __init__(self, src=0, fps=30, width=1280, height=720):
        self.stream = cv2.VideoCapture(src)

        fourcc_yuy2 = cv2.VideoWriter_fourcc(*"YUY2")
        self.stream.set(cv2.CAP_PROP_FOURCC, fourcc_yuy2)
        actual_fourcc = int(self.stream.get(cv2.CAP_PROP_FOURCC))
        # Kamera YUY2'yi reddettiyse MJPG'ye dön
        if actual_fourcc != fourcc_yuy2:
            self.stream.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))

        # --- Çözünürlük (kullanıcı seçimine göre) ---
        ...
        # İlk kare için bekleme süresi hesabında kullanılacak FPS
        self._sleep_interval = max(0.001, 1.0 / max(fps, 1))
    ...
    def update(self):
        while not self.stopped:
            ...
            with self.lock:
                ...
                self.frame   = frame
            # CPU'yu %100 kullanmasını önlemek için FPS'e göre kısa uyku
            time.sleep(self._sleep_interval * 0.5)
...
class WebcamPreviewWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master, engine: "RecordingEngine"):
        ...
        hdr.pack(fill="x")
        ctk.CTkLabel(hdr, text="Webcamı kapatmak için X'e tıklayın",
            ...
            if ret and frame is not None:
                try:
                    ...
                    if win_w > 10 and win_h > 10:
                        ...
                        if nw > 0 and nh > 0:
                            # Büyütme → LANCZOS4, küçültme → AREA
...
class RecordingEngine:
    def __init__(self):
        ...
        self.pip_pixel_size  = (240, 135)
        self.webcam_res      = (1280, 720)
    ...
    def start(self):
        ...
        if self.webcam_on:
            ww, wh = self.webcam_res
            self.cap_stream = WebcamVideoStream(
                src=self.cam_idx, fps=self.fps, width=ww, height=wh
            ).start()
            time.sleep(0.5)
    ...
    def _get_mon(self):
        with mss_lib.mss() as s:
            if self.region:
                x, y, w, h = self.region
                # FFmpeg (yuv420p) hatasını önlemek için genişlik ve yüksekliği çift sayıya sabitliyoruz:
                w = w - (w % 2)
                h = h - (h % 2)
                return {"left": x, "top": y, "width": w, "height": h}
            
            m = s.monitors[1]
            # Tam ekran çözünürlüğünün de (nadir de olsa) tek sayı olma ihtimaline karşı:
            mw, mh = m["width"], m["height"]
            mw = mw - (mw % 2)
            mh = mh - (mh % 2)
            return {"left": m["left"], "top": m["top"], "width": mw, "height": mh}
    ...
    def _loop_screen(self):
        ...
        # Kayıt sonrası optimize etmek için _recompress() çağrısı kullanılabilir.
        if FFMPEG:
            ffcmd = [
                "ffmpeg", "-y",
                # Giriş: ham video — PTS'siz pipe; zaman damgasını ffmpeg hesaplar
                "-f", "rawvideo", "-vcodec", "rawvideo",
                "-s", f"{W}x{H}", "-pix_fmt", "bgr24", "-r", str(self.fps),
                "-i", "pipe:",
                # H.264 gerçek zamanlı kodlama
                "-vcodec", "libx264", "-preset", "ultrafast", "-tune", "zerolatency",
                "-crf", str(self.quality), "-pix_fmt", "yuv420p",
                "-colorspace", "bt709", "-color_trc", "bt709",
                "-color_primaries", "bt709", "-color_range", "tv",
                # vsync cfr: sabit FPS → kare atlama/tekrarlama yerine interpolasyon
                "-vsync", "cfr",
                "-threads", "0",
                self._vid_tmp
            ]
                ...
                if frames_to_write > 0:
                    ...
                    if pip_active:
                        ...
                        if ret and wf is not None:
                            ...
                            src_h, src_w = wf.shape[:2]
                            # Büyütme → LANCZOS4 (keskin kenar), küçültme → AREA (moiré yok)
                            interp = cv2.INTER_LANCZOS4 if (pw * ph > src_w * src_h) else cv2.INTER_AREA
                            wf = cv2.resize(wf, (pw, ph), interpolation=interp)
                            border = 3
    ...
    def _apply_wm(self, frame, W, H):
        """
        Filigranı kareye uygular.

        İyileştirme: filigran içeriği (metin, görsel, konum, saydamlık) değişmediği
        sürece overlay PIL → NumPy dönüşümü yalnızca bir kez yapılır; sonraki
        karelerde önbellekteki NumPy dizisi doğrudan kullanılır. Bu yaklaşım,
        her karede tekrarlanan PIL → RGBA → alpha_composite → NumPy dönüşümünden
        kaynaklanan CPU yükünü önemli ölçüde azaltır.
        """
        # Önbellek anahtarı: filigranı etkileyen tüm parametrelerin özeti
        cache_key = (
            self.wm_text, id(self.wm_image),
            self.wm_pos, round(self.wm_opacity, 3), W, H
        )

        if self._wm_cache is None or self._wm_cache_key != cache_key:
            # --- Önbellek oluştur ---
            if self._wm_font is None:
                try:
                    self._wm_font = ImageFont.truetype(
                        "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 22
                    )
                except Exception:
                    self._wm_font = ImageFont.load_default()
            fnt = self._wm_font
            ...
            if self.wm_image:
                try:
                    ...
                    pass
            # RGBA overlay'i önceden float32 NumPy dizisine çevir
            ov_np              = np.array(ov, dtype=np.float32)
            self._wm_alpha     = ov_np[:, :, 3:4] / 255.0   # (H, W, 1)
            self._wm_rgb       = ov_np[:, :, :3]             # (H, W, 3)  — RGB sırası
            self._wm_cache     = True
            self._wm_cache_key = cache_key

        # --- Önbellekten uygula (her kare) ---
        # frame BGR; filigran RGB → kanal sırasını düzelt
        wm_bgr = self._wm_rgb[:, :, ::-1]
        blended = (
            frame.astype(np.float32) * (1.0 - self._wm_alpha) +
            wm_bgr * self._wm_alpha
        )
    ...
    def _merge(self):
        ...
        if not has_v: return
        crf = str(getattr(self, "quality", 18))
        if has_a and FFMPEG:
            try:
                # ultrafast ile kaydedilen videoyu medium preset ile yeniden encode et
                # → aynı CRF değerinde çok daha küçük ve kaliteli çıktı
                cmd = [
                    "ffmpeg", "-y",
                    "-i", self._vid_tmp, "-i", self._aud_tmp,
                    "-c:v", "libx264", "-preset", "medium",
                    "-crf", crf, "-pix_fmt", "yuv420p",
                    "-c:a", "aac", "-b:a", "192k",
                    "-af", "aresample=async=1",
                    "-movflags", "+faststart",
                    "-shortest", self.last_path
                ]
        ...
        if FFMPEG:
            try:
                # Ses yoksa sadece video encode et (copy yerine medium preset)
                cmd = [
                    "ffmpeg", "-y", "-i", self._vid_tmp,
                    "-c:v", "libx264", "-preset", "medium",
                    "-crf", crf, "-pix_fmt", "yuv420p",
                    "-movflags", "+faststart",
                    self.last_path
                ...
                r = subprocess.run(cmd, capture_output=True, timeout=300, **_no_window())
                if r.returncode == 0:
                    os.remove(self._vid_tmp); return
            except: pass
...
class VideoTimelineEditor(ctk.CTkFrame):
    _CLIP_Y  = 26;  _CLIP_H  = 54
    _AUDIO_Y = 90;  _AUDIO_H = 24
    _TEXT_Y  = 124; _TEXT_H  = 22
    _SHAPE_Y = 154; _SHAPE_H = 22
    ...
    def __init__(self, master):
        ...
        self.selected_text  = None   # seçili metin overlay
        self.selected_asset = None   # seçili şekil/görsel asset
        self.playing        = False
        ...
        self._drag_text     = None   # sürüklenen metin
        self._drag_asset    = None   # sürüklenen asset
    ...
    def _build(self):
        ...
        toolbar_buttons = [
            ...
            ("🖼️ Şekil/Görsel", "assets", self._open_asset_editor, "#0d6efd", "#0b5ed7"),
        ]
        self._toolbar_btns = {}
        ...
        ctk.CTkButton(z_f, text="➕", width=26, height=22, fg_color=ACCENT, command=self._tl_zoom_in).pack(side="left", padx=2)
        ctk.CTkButton(z_f, text="➖", width=26, height=22, fg_color=ACCENT2, command=self._tl_zoom_out).pack(side="left", padx=2)
        tl_wrap = ctk.CTkFrame(tl_card, fg_color="transparent")
        tl_wrap.pack(fill="x", padx=8, pady=(2, 8))
        self.tl_canvas = tk.Canvas(tl_wrap, bg=CARD, height=200, highlightthickness=2, highlightbackground=ACCENT)
        ...
        prop_tabs.add("🎵 Ses")
        prop_tabs.add("🎬 Geçiş")
        prop_tabs.add("🔷 Şekiller")
        
        # Bu sekmenin içeriğini oluşturuyoruz
        tab_shapes = ctk.CTkFrame(prop_tabs.tab("🔷 Şekiller"), fg_color="transparent")
        tab_shapes.pack(fill="both", expand=True)

        ctk.CTkLabel(tab_shapes, text="🔷 Eklenen Şekiller/Görseller",
            font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2).pack(anchor="w", padx=8, pady=(8, 4))

        # Yeni Ekle Butonu
        ctk.CTkButton(tab_shapes, text="➕ Yeni Şekil Ekle", height=30,
            fg_color="#0d6efd", hover_color="#0b5ed7",
            command=self._open_asset_editor).pack(fill="x", padx=8, pady=(0, 6))

        # Liste Alanı (Scrollable)
        self.asset_list_frame = ctk.CTkScrollableFrame(tab_shapes, fg_color=CARD, corner_radius=6)
        self.asset_list_frame.pack(fill="both", expand=True, padx=8, pady=4)
    ...
    def _update_preview_at(self, tpos):
        ...
        def _show(frame):
            ...
            try:
                ...
                # FIX: Asset döngüsü text overlay döngüsünün dışına taşındı
                # Önceki hatada for asset iç içe yerleşmişti, metin yoksa asset da çizilmiyordu
                for asset in self.assets:
                    if asset.start_time <= tpos <= asset.end_time:
                        try:
                            if asset.type == "rect":
                                ax = int(asset.x_pct * pw); ay = int(asset.y_pct * ph)
                                aw = int(asset.w_pct * pw); ah = int(asset.h_pct * ph)
                                draw.rectangle([ax, ay, ax+aw, ay+ah], outline=asset.color, width=max(1, asset.thickness))
                            elif asset.type == "circle":
                                ax = int(asset.x_pct * pw); ay = int(asset.y_pct * ph)
                                aw = int(asset.w_pct * pw); ah = int(asset.h_pct * ph)
                                draw.ellipse([ax, ay, ax+aw, ay+ah], outline=asset.color, width=max(1, asset.thickness))
                            elif asset.type == "line":
                                ax1 = int(asset.x_pct * pw); ay1 = int(asset.y_pct * ph)
                                ax2 = int((asset.x_pct + asset.w_pct) * pw)
                                draw.line([ax1, ay1, ax2, ay1+2], fill=asset.color, width=max(1, asset.thickness))
                            elif asset.type == "image" and os.path.exists(asset.path):
                                from PIL import Image as PIL_Image
                                img = PIL_Image.open(asset.path).convert("RGBA")
                                iw = int(asset.w_pct * pw); ih = int(asset.h_pct * ph)
                                img = img.resize((iw, ih), PIL_Image.LANCZOS)
                                ix = int(asset.x_pct * pw); iy = int(asset.y_pct * ph)
                                pil.paste(img, (ix, iy), img)
                        except Exception: pass
    ...
    def _render_timeline(self):
        ...
        TY, TH   = self._TEXT_Y,  self._TEXT_H
        SY, SH   = self._SHAPE_Y, self._SHAPE_H
        total_canvas_h = SY + SH + 8

        c.configure(scrollregion=(0, 0, total_w, total_canvas_h))

        # ── Şerit arka planları ─────────────────────────────────────
        c.create_rectangle(0, 0,  total_w, 22,          fill="#1c2128", outline="")
        c.create_rectangle(0, CY, total_w, CY + CH,     fill="#0d1117", outline="")
        c.create_rectangle(0, AY, total_w, AY + AH,     fill="#0a1a15", outline="")
        c.create_rectangle(0, TY, total_w, TY + TH,     fill="#1a150a", outline="")
        c.create_rectangle(0, SY, total_w, SY + SH,     fill="#0a0a1a", outline="")

        # Şerit ayırıcı çizgiler
        for y in (CY, AY, TY, SY, SY + SH):
            c.create_line(0, y, total_w, y, fill=BORDER, width=1)

        # ── Şerit etiketleri ────────────────────────────────────────
        c.create_text(3, CY + CH // 2, text="🎬 VİDEO", fill=TXT2, anchor="w",
                      font=("Arial", 8, "bold"))
        c.create_text(3, AY + AH // 2, text="🎵 SES",   fill=TXT2, anchor="w",
                      font=("Arial", 8, "bold"))
        c.create_text(3, TY + TH // 2, text="T METİN",  fill="#fbbf24", anchor="w",
                      font=("Arial", 8, "bold"))
        c.create_text(3, SY + SH // 2, text="◆ ŞEKİL",  fill="#60a5fa", anchor="w",
                      font=("Arial", 8, "bold"))
        ...
        for s in range(0, int(self.timeline_dur) + step, step):
            x = s * self.px_per_sec
            c.create_line(x, 16, x, 22, fill=BORDER)
            mm, ss = divmod(s, 60)
            c.create_text(x + 2, 11, text=f"{mm:02d}:{ss:02d}", fill=TXT2,
        ...
        for i, clip in enumerate(sorted_clips):
            x0  = clip.timeline_start * self.px_per_sec
            x1  = clip.timeline_end   * self.px_per_sec
            col = COLS[i % len(COLS)]
            is_sel  = (clip is self.selected_clip)
            outline = "#ffffff" if is_sel else BORDER
            width   = 2 if is_sel else 1
            c.create_rectangle(x0, CY, x1, CY + CH,
                               fill=col, outline=outline, width=width,
                               tags=("clip", f"clip_{i}"))
            name = (clip.name[:18] + "…") if len(clip.name) > 18 else clip.name
            if clip.mute: name = "🔇 " + name
            c.create_text(x0 + 6, CY + 10, text=name,
                          fill="white", anchor="w", font=("Arial", 9, "bold"),
                          tags=(f"clip_{i}",))
            dur_mm, dur_ss = divmod(int(clip.clip_duration), 60)
            spd_txt = f"  {clip.speed}×" if clip.speed != 1.0 else ""
            c.create_text(x0 + 6, CY + 24,
                          text=f"{dur_mm:02d}:{dur_ss:02d}{spd_txt}",
                          fill="#c8d0da", anchor="w", font=("Arial", 9))
            if clip.scale_w > 0:
                c.create_text(x0 + 6, CY + 38,
                              text=f"↔ {clip.scale_w}×{clip.scale_h}",
                              fill="#c8d0da", anchor="w", font=("Arial", 8))
            # Trim tutamaçları
            c.create_rectangle(x0, CY, x0 + 7, CY + CH,
                               fill="#fff" if is_sel else "#888", outline="",
                               tags=(f"triml_{i}",))
            c.create_rectangle(x1 - 7, CY, x1, CY + CH,
                               fill="#fff" if is_sel else "#888", outline="",
                               tags=(f"trimr_{i}",))

        # ── Geçiş efekti işaretleri ─────────────────────────────────
        for i in range(len(sorted_clips) - 1):
            ...
            r = 10
            if trans.is_cut:
                c.create_line(jx, CY + 4, jx, CY + CH - 4,
                              fill="#ffffff", width=2, dash=(4, 3),
                              tags=(f"trans_{i}",))
                c.create_text(jx, CY - 6, text="✂️", fill="#aaa",
                              font=("Arial", 7), tags=(f"trans_{i}",))
            else:
                pts = [jx, cy_mid - r, jx + r, cy_mid,
                       jx, cy_mid + r, jx - r, cy_mid]
                c.create_polygon(pts, fill="#f59e0b", outline="#ffffff",
                                 width=1, tags=(f"trans_{i}",))
                c.create_text(jx, cy_mid, text=trans.emoji,
                              font=("Arial", 8), fill="#000",
                              tags=(f"trans_{i}",))
                c.create_text(jx, CY + CH + 4,
                              text=f"{trans.duration:.1f}s", fill="#f59e0b",
                              font=("Arial", 7), tags=(f"trans_{i}",))
        ...
        for i, at in enumerate(self.audio_tracks):
            ...
            c.create_rectangle(x0, AY, x1, AY + AH,
                               fill="#2d6a4f",
                               outline="#fff" if is_sel else BORDER,
                               width=2 if is_sel else 1,
                               tags=(f"audio_{i}",))
            c.create_text(x0 + 4, AY + AH // 2,
                          text=f"🎵 {at.name[:18]}  {at.trim_start:.1f}→{at.trim_end:.1f}s",
                          fill="white", anchor="w", font=("Arial", 8),
                          tags=(f"audio_{i}",))
            # Trim tutamaçları
            c.create_rectangle(x0, AY, x0 + 6, AY + AH,
                               fill="#fff" if is_sel else "#555", outline="",
                               tags=(f"audio_triml_{i}",))
            c.create_rectangle(x1 - 6, AY, x1, AY + AH,
                               fill="#fff" if is_sel else "#555", outline="",
                               tags=(f"audio_trimr_{i}",))

        # ── METİN şeridi ─────────────────────────────────────────────
        for i, ov in enumerate(self.text_overlays):
            x0     = ov.start_time * self.px_per_sec
            x1     = ov.end_time   * self.px_per_sec
            is_sel = (ov is self.selected_text)
            c.create_rectangle(x0, TY, x1, TY + TH,
                               fill="#92400e" if not is_sel else "#f59e0b",
                               outline="#fbbf24" if is_sel else "#78350f",
                               width=2 if is_sel else 1,
                               tags=(f"text_{i}",))
            label = f"T  {ov.text[:14]}{'…' if len(ov.text) > 14 else ''}"
            c.create_text(x0 + 4, TY + TH // 2,
                          text=label, fill="#fef3c7", anchor="w",
                          font=("Arial", 8, "bold"), tags=(f"text_{i}",))
            # Trim tutamaçları
            c.create_rectangle(x0, TY, x0 + 5, TY + TH,
                               fill="#fbbf24" if is_sel else "#78350f", outline="")
            c.create_rectangle(x1 - 5, TY, x1, TY + TH,
                               fill="#fbbf24" if is_sel else "#78350f", outline="")
        ...
        for i, asset in enumerate(self.assets):
            x0     = asset.start_time * self.px_per_sec
            x1     = asset.end_time   * self.px_per_sec
            is_sel = (asset is self.selected_asset)
            base_col = _shape_colors.get(asset.type, "#1e40af")
            c.create_rectangle(x0, SY, x1, SY + SH,
                               fill=base_col if not is_sel else "#3b82f6",
                               outline="#93c5fd" if is_sel else "#1e3a8a",
                               width=2 if is_sel else 1,
                               tags=(f"shape_{i}",))
            _icons = {"rect": "▭", "circle": "⭕", "line": "—", "image": "🖼"}
            icon  = _icons.get(asset.type, "◆")
            label = f"{icon} {asset.name[:14]}{'…' if len(asset.name) > 14 else ''}"
            c.create_text(x0 + 4, SY + SH // 2,
                          text=label, fill="#dbeafe", anchor="w",
                          font=("Arial", 8, "bold"), tags=(f"shape_{i}",))
            # Trim tutamaçları
            c.create_rectangle(x0, SY, x0 + 5, SY + SH,
                               fill="#93c5fd" if is_sel else "#1e40af", outline="")
            c.create_rectangle(x1 - 5, SY, x1, SY + SH,
                               fill="#93c5fd" if is_sel else "#1e40af", outline="")

        # ── Playhead (en üste) ───────────────────────────────────────
        px = self.playhead_pos * self.px_per_sec
        c.create_line(px, 0, px, total_canvas_h, fill=RED, width=2,
                      tags=("playhead",))
        c.create_polygon(px - 7, 0, px + 7, 0, px, 11,
    ...
    def _tl_click(self, e):
        ...
        if AY <= e.y <= AY + AH:
            for i, at in enumerate(self.audio_tracks):
                ...
                x1 = x0 + at.clip_duration * self.px_per_sec
                if x0 <= cx <= x1:
                    self.selected_audio = at
                    self.selected_clip  = None
                    self.selected_text  = None
                    self.selected_asset = None
                    if cx <= x0 + 6:   self._drag_mode = "trim_l"
                    elif cx >= x1 - 6: self._drag_mode = "trim_r"
                    else:              self._drag_mode = "move"; self._drag_offset = cx - x0
                    self._drag_audio = at
                    self._drag_clip  = None
                    self._drag_text  = None
                    self._drag_asset = None
                    self._update_audio_props()
                    self._render_timeline()
                    return
        ...
        if TY <= e.y <= TY + TH:
            for i, ov in enumerate(self.text_overlays):
                ...
                x1 = ov.end_time   * self.px_per_sec
                if x0 <= cx <= x1:
                    self.selected_text  = ov
                    self.selected_clip  = None
                    self.selected_audio = None
                    self.selected_asset = None
                    if cx <= x0 + 5:   self._drag_mode = "trim_l"
                    elif cx >= x1 - 5: self._drag_mode = "trim_r"
                    else:              self._drag_mode = "move"; self._drag_offset = cx - x0
                    self._drag_text  = ov
                    self._drag_clip  = None
                    self._drag_audio = None
        ...
        # ── Boş alana tıklama → seçimi temizle + seek ───────────────
        self._drag_clip  = None
        self._drag_audio = None
        self._drag_text  = None
        self._drag_asset = None
    ...
    def _build_clip_vf(self, eff, dur=None):
        ...
        # dur burada clip_duration (hız uygulanmış gerçek çıktı süresi) olmalı
        self._fade_in_for_af  = 0.0
        self._fade_out_for_af = 0.0
        if dur is not None:
            fi = eff.get("fade_in",  0.0)
            fo = eff.get("fade_out", 0.0)
            if fi > 0:
                vf.append(f"fade=t=in:st=0:d={fi}")
                self._fade_in_for_af  = fi   # ses afade için sakla
            if fo > 0:
                vf.append(f"fade=t=out:st={max(0, dur - fo):.3f}:d={fo}")
                self._fade_out_for_af = fo   # ses afade için sakla
    ...
    def _open_clip_effects(self):
        ...
        if os.path.exists(icon_path):
            try:
                ...
                self.after(200, lambda: win.iconbitmap(icon_path))
            except Exception as e:
                print(f"İkon yükleme hatası: {e}")
        else:
            print("Uyarı: AQ2.ico dosyası bulunamadı.")
        ...
        def _run_preview(cur_eff):
            ...
            def _safe_configure(**kwargs):
                """Pencere ve widget hâlâ geçerliyse configure çağır."""
                try:
                    if win.winfo_exists() and preview_lbl.winfo_exists():
                        preview_lbl.configure(**kwargs)
                except Exception:
                    pass
            ...
            try:
                ...
                if r.returncode == 0 and os.path.exists(tmp):
                    ...
                    if win.winfo_exists():
                        win.after(0, lambda i=ctk_img: _safe_configure(image=i, text=""))
                else:
                    if win.winfo_exists():
                        win.after(0, lambda: _safe_configure(text="⚠️  Önizleme oluşturulamadı.", image=""))
            except Exception:
                if win.winfo_exists():
                    win.after(0, lambda: _safe_configure(text="⚠️  FFmpeg hatası.", image=""))
    ...
    def _resolve_clip_placement(self, clip, new_start, new_end, track_list):
            ...
            # Hem başlangıç hem bitiş zamanı için dinamik okuma fonksiyonları eklendi
            def _get_start(t): return t.timeline_start if hasattr(t, "timeline_start") else t.start_time
            ...
            for other in track_list:
                ...
                o_s = _get_start(other) * px_s   # <-- DÜZELTİLDİ
            ...
            if candidates:
                ...
                for cand in candidates:
                    ...
                    if not any(cand < _get_end(o) and cand_end > _get_start(o) for o in track_list if o is not clip): # <-- DÜZELTİLDİ
                        new_start = cand
            ...
            for other in track_list:
                ...
                o_start = _get_start(other) # <-- DÜZELTİLDİ
                o_end = _get_end(other)
            
                if new_start < o_end and new_end > o_start: # <-- DÜZELTİLDİ
                    # Çakışma tespit edildi, en yakın güvenli kenara it
                    if abs(new_start - o_end) <= abs(o_start - new_end): # <-- DÜZELTİLDİ
                        new_start = o_end
                    else:
                        new_start = o_start - duration # <-- DÜZELTİLDİ
    ...
    def _open_asset_editor(self):
        def on_save(asset):
            self.assets.append(asset)
            self._update_timeline_dur()
            self._render_timeline()
            self._update_asset_list()  # FIX: Şekiller/Görseller listesi de güncellendi
            messagebox.showinfo("✓", f"{asset.name} timeline'a eklendi.")
        AssetEditorDialog(self, on_save)

    def _edit_asset(self, asset):
        """Asset'i mevcut değerleriyle diyalogda aç, kaydedince yerinde güncelle."""
        def on_save(updated):
            try:
            ...
            self._update_asset_list()
            self._render_timeline()
        AssetEditorDialog(self, on_save, existing=asset)

    def _update_asset_list(self):
        """Şekiller/Görseller listesini UI'da günceller."""
        if not hasattr(self, 'asset_list_frame'): return
        for w in self.asset_list_frame.winfo_children(): w.destroy()

        if not self.assets:
            ctk.CTkLabel(self.asset_list_frame, text="Henüz şekil eklenmedi.", text_color=TXT2).pack(pady=10)
            return

        for asset in self.assets:
            r = ctk.CTkFrame(self.asset_list_frame, fg_color=CARD2, corner_radius=6)
            r.pack(fill="x", pady=2, padx=4)
            ctk.CTkLabel(r, text=f"{asset.name} | {asset.start_time:.1f}s - {asset.end_time:.1f}s",
                         font=ctk.CTkFont("Arial", 10), text_color=TXT).pack(side="left", padx=6, pady=4)
            # FIX: Düzenle butonu eklendi
            ctk.CTkButton(r, text="✏", width=28, height=24, fg_color=ACCENT2,
                          command=lambda a=asset: self._edit_asset(a)).pack(side="right", padx=2, pady=4)
            ctk.CTkButton(r, text="🗑", width=28, height=24, fg_color=RED,
                          command=lambda a=asset: self._delete_asset(a)).pack(side="right", padx=2, pady=4)

    def _delete_asset(self, asset):
        """Asset listesinden siler ve UI'ı günceller."""
        ...
        def _run():
            try:
                ...
                for i, clip in enumerate(sorted_clips):
                    ...
                    # ── Ses stream'i var mı? MTS/AVCHD için codec_name de kontrol et ──
                    _ap = subprocess.run(
                    ...
                    clip_has_audio = bool(_ap.stdout.strip())

                    # ── Şekil & Görsel Varlıkları (Assets) ──────────────────────
                    seg_start = clip.timeline_start
                    seg_end   = clip.timeline_end
                    active_assets = [a for a in self.assets if a.start_time < seg_end and a.end_time > seg_start]

                    if active_assets:
                        # Gerçek video boyutunu al — drawbox sabit piksel gerektirir
                        try:
                            _vp = subprocess.run(
                            ...
                            _dims = _vp.stdout.decode().strip().split(",")
                            _vid_w = int(_dims[0]) if len(_dims) >= 2 else 1920
                            _vid_h = int(_dims[1]) if len(_dims) >= 2 else 1080
                        except Exception:
                            _vid_w, _vid_h = 1920, 1080
                        # Seçilen çözünürlük varsa onu kullan
                        if chosen_res != "Orijinal":
                            _rw, _rh = chosen_res.split("x")
                            _vid_w, _vid_h = int(_rw), int(_rh)
                        asset_vf = build_asset_vf(active_assets, seg_start, clip.clip_duration,
                                                   vid_w=_vid_w, vid_h=_vid_h)
                        for part in asset_vf:
                            if ";" in part or "movie=" in part:
                                v_filters.insert(0, "__FC__:" + part)
                            else:
                                v_filters.append(part)

                    # ── Hız ────────────────────────────────────────
                    if clip.speed != 1.0:
                        v_filters.append(f"setpts={1/clip.speed:.4f}*(PTS-STARTPTS)")
                        spd = clip.speed
                    ...
                    if chosen_res != "Orijinal":
                        ...
                        # force_divisible_by=2: yuv420p için çift boyut zorunlu
                        v_filters.append(
                            f"scale={rw}:{rh}:force_original_aspect_ratio=decrease"
                            f":force_divisible_by=2"
                        )
                        v_filters.append(f"pad={rw}:{rh}:(ow-iw)/2:(oh-ih)/2:color=black")
                    elif clip.scale_w > 0 and clip.scale_h > 0:
                        sw = clip.scale_w - (clip.scale_w % 2)
                        sh = clip.scale_h - (clip.scale_h % 2)
                        v_filters.append(f"scale={sw}:{sh}")

                    # ── Video Efektleri (renk, filtre, dönüşüm, solma) ──────────
                    clip_eff = getattr(clip, "effects", {})
                    real_out_dur = clip.clip_duration
                    if clip_eff:
                        eff_vf = self._build_clip_vf(clip_eff, dur=real_out_dur)
                        v_filters.extend(eff_vf)
                        fi_af = getattr(self, "_fade_in_for_af",  0.0)
                        fo_af = getattr(self, "_fade_out_for_af", 0.0)
                        if fi_af > 0:
                            a_filters.append(f"afade=t=in:st=0:d={fi_af}")
                        if fo_af > 0:
                            a_filters.append(f"afade=t=out:st={max(0, real_out_dur - fo_af):.3f}:d={fo_af}")
                    # ── Metin katmanları ────────────────────────────
                    if os.name == 'nt':
                    ...
                    for ov in self.text_overlays:
                        ...
                        if rel_e > 0 and rel_s < real_out_dur:
                            rs   = max(0.0, rel_s)
                            re   = min(real_out_dur, rel_e)
                            safe = ov.text.replace("\\","\\\\").replace("'","\\'").replace(":","\\:").replace(",", "\\,")
                            hx = ov.color.lstrip("#").upper().zfill(6)
                    ...
                    # ── ffmpeg komutu ────────────────────────────────────────────
                    # Filtre sırası kritik:
                    # 1) setpts  2) scale+pad  3) format=yuv420p  4) drawbox/geq/drawtext  5) asset overlay
                    # format=yuv420p scale/pad'den HEMEN SONRA gelmelidir —
                    # aksi takdirde yuv422p/yuv444p akışı libx264'e ulaşır → EINVAL (-22)

                    fc_chains = []
                    timing_vf  = []   # setpts
                    scale_vf   = []   # scale, pad
                    overlay_vf = []   # drawbox, geq, drawtext (piksel üzerinde çalışır)

                    for vf_part in v_filters:
                        if vf_part.startswith("__FC__:"):
                            fc_chains.append(vf_part[7:])
                        elif vf_part.startswith("setpts"):
                            timing_vf.append(vf_part)
                        elif vf_part.startswith("scale") or vf_part.startswith("pad"):
                            scale_vf.append(vf_part)
                        else:
                            overlay_vf.append(vf_part)

                    # Sıralı basit filtre zinciri: timing → scale/pad → format → overlay
                    simple_vf = timing_vf + scale_vf + ["format=yuv420p"] + overlay_vf
                    ...
                    if fc_chains:
                        fc_str = ";".join(fc_chains)
                        if simple_vf:
                            fc_str += ";[vout]" + ",".join(simple_vf) + "[vout2]"
                            cmd += ["-filter_complex", fc_str, "-map", "[vout2]"]
                        else:
                            cmd += ["-filter_complex", fc_str, "-map", "[vout]"]
                    else:
                    ...
                    if r.returncode == 0:
                        ...
                        stderr_full = r.stderr.decode("utf-8", errors="replace")
                        # Gerçek ffmpeg hata satırını bul (başında "Error" veya son satırlar)
                        lines = [l for l in stderr_full.splitlines() if l.strip()]
                        err_lines = [l for l in lines if any(k in l for k in ("Error","error","Invalid","No such","failed","Failed","Cannot","cannot","Unknown","unknown"))]
                        err_display = "\n".join(err_lines[-6:]) if err_lines else "\n".join(lines[-8:])
                        cmd_str = " ".join(cmd[:12]) + " ..."  # kısa komut özeti
                        self.after(0, lambda e=err_display, c=cmd_str: self.export_status.configure(
                            text=f"✗ Segment hatası:\n{e}\n\ncmd: {c}", text_color=RED))
                        return
                ...
                if len(processed) == 1:
                    ...
                    if not has_fx:
                        ...
                        # ── Video: xfade zinciri (doğrusal zincir) ────────────────
                        # ── Ses:   adelay+atrim+amix (acrossfade yerine; zaman damgası kayması olmaz)
                        v_parts = []   # sadece video xfade zinciri
                        a_parts = []   # ses delay/trim/amix parçaları
                        prev_lbl       = "[0:v]"
                        cumulative_dur = seg_durs[0]

                        # Her segmentin ses akışının timeline üzerindeki başlangıç zamanını hesapla
                        # Segment i, (cumulative_dur_i - tdur_i) noktasında başlamalı (overlap nedeniyle)
                        seg_offsets = [0.0]  # seg 0 her zaman 0'dan başlar
                        tmp_cum = seg_durs[0]
                        for i, (ca, cb) in enumerate(pairs):
                            t    = self.transitions.get((id(ca), id(cb)), TransitionData("cut", 0.5))
                            tdur = t.duration if not t.is_cut else 0.0
                            tdur = min(tdur, seg_durs[i] * 0.85, seg_durs[i+1] * 0.85)
                            tdur = max(0.0, tdur)
                            offset = max(0.0, tmp_cum - tdur)
                            seg_offsets.append(offset)
                            tmp_cum = offset + seg_durs[i+1]

                        total_out_dur = tmp_cum  # toplam çıktı süresi
                        ...
                        for i, (ca, cb) in enumerate(pairs):
                            ...
                            tdur   = min(tdur, seg_durs[i] * 0.85, seg_durs[i+1] * 0.85)
                            tdur   = max(0.0, tdur)
                            offset = max(0.0, cum2 - tdur)

                            is_last = (i == len(pairs) - 1)
                            v_out   = "[vout]" if is_last else f"[xv{i}]"
                            next_v  = f"[{i+1}:v]"

                            # Video xfade (tdur == 0 ise sadece concat mantığıyla çalışır; offset aynı)
                            ...
                            if tdur > 0:
                                ...
                                v_parts.append(
                                    f"{prev_lbl}{next_v}xfade=transition=fade"
                                    f":duration=0.001:offset={offset:.3f}{v_out}"
                                )
                            prev_lbl = v_out
                            cum2     = offset + seg_durs[i+1]

                        # Ses: her segmenti doğru zaman damgasına yerleştir, sonra amix
                        n_segs = len(audio_ready)
                        for si in range(n_segs):
                            delay_ms = int(seg_offsets[si] * 1000)
                            # Ses akışını kırp + geciktir
                            a_parts.append(
                                f"[{si}:a]asetpts=PTS-STARTPTS,adelay={delay_ms}|{delay_ms}[da{si}]"
                            )
                        mix_ins = "".join(f"[da{si}]" for si in range(n_segs))
                        # amix: duration=longest → en uzun sese göre tamamlar
                        a_parts.append(
                            f"{mix_ins}amix=inputs={n_segs}:duration=longest:normalize=0[aout]"
                        )
                        ...
                        if rx.returncode != 0:
                            ...
                            with open(lst, "w", encoding="utf-8") as ff:
                                for p in audio_ready:   # normalize edilmiş dosyaları kullan
                ...
                err_msg = str(ex)  # Hatayı string'e çevirip kalıcı bir değişkene alıyoruz
                self.after(0, lambda e=err_msg: self.export_status.configure(
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Aptoza"); self.geometry("1280x740"); self.minsize(1050, 640); self.configure(fg_color=BG)
        # --- GÜVENLİ İKON YÜKLEME ---
        # Dosyanın tam yolunu oluşturuyoruz
        current_dir = os.path.dirname(os.path.abspath(__file__))
        icon_path = os.path.join(current_dir, "AQ2.ico")
    ...
    def _build_ui(self):
        ...
        logo = ctk.CTkLabel(self.sidebar_inner, text="Aptoza", font=ctk.CTkFont("Arial", 24, "bold"), text_color=TXT); logo.pack(pady=(5, 5), padx=16)
        ctk.CTkLabel(self.sidebar_inner, text="V1.2", font=ctk.CTkFont("Arial", 11), text_color=TXT2).pack()
        ...
        ctk.CTkFrame(self.sidebar_inner, height=1, fg_color=BORDER).pack(fill="x", padx=5, pady=3)
        self.btn_show_tools = ctk.CTkButton(self.sidebar_inner, text="🛠️ Araç ve Zoom Paneli", height=36, fg_color=ACCENT2, hover_color="#FF0000", command=self.tool_window.show)
        self.btn_show_tools.pack(fill="x", padx=10, pady=(3, 3))
        ctk.CTkFrame(self.sidebar_inner, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=5)
        ...
        ctk.CTkButton(self.sidebar_inner, text="📷  Ekran Görüntüsü Al", height=34, fg_color=ACCENT2, hover_color=BORDER, font=ctk.CTkFont("Arial", 12, "bold"), anchor="w", command=self._take_screenshot).pack(fill="x", padx=10, pady=(0, 8))
        ...
        self.lbl_timer_side  = ctk.CTkLabel(self.sidebar_inner, text="00:00:00", font=ctk.CTkFont("Courier", 26, "bold"), text_color=ACCENT); self.lbl_timer_side.pack(pady=4)
        self.btn_start = ctk.CTkButton(self.sidebar_inner, text="⏺️ Kayıt Başlat", height=36, fg_color=ACCENT2, hover_color=RED, font=ctk.CTkFont("Arial", 13, "bold"), anchor="w", command=self._start_recording)
        self.btn_start.pack(fill="x", padx=10, pady=(5, 3))
        self.btn_pause = ctk.CTkButton(self.sidebar_inner, text="⏸️ Duraklat", height=36, fg_color=ACCENT2, hover_color=YELLOW, text_color="#000", font=ctk.CTkFont("Arial", 13, "bold"), anchor="w", state="disabled", command=self._pause_resume)
        self.btn_pause.pack(fill="x", padx=10, pady=3)
        self.btn_stop = ctk.CTkButton(self.sidebar_inner, text="⏹️ Durdur", height=36, fg_color=ACCENT2, hover_color=PURPLE, font=ctk.CTkFont("Arial", 13, "bold"), anchor="w", state="disabled", command=self._stop_recording)
        self.btn_stop.pack(fill="x", padx=10, pady=3)
        # ── Kayıt sırasında canlı mikrofon mute butonu ──
        self.btn_mic_mute = ctk.CTkButton(
            self.sidebar_inner, text="🎤 Mikrofon Açık", height=36,
            fg_color=ACCENT2, hover_color=RED,
            font=ctk.CTkFont("Arial", 12, "bold"), anchor="w",
            state="disabled", command=self._toggle_mic_mute
        )
        self.btn_mic_mute.pack(fill="x", padx=10, pady=(3, 3))
        ctk.CTkFrame(self.sidebar_inner, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=5)
        ...
        # Bind mousewheel ONLY when mouse is over the sidebar (Enter/Leave ile sınırla)
        def _on_sidebar_mousewheel(event):
            self.sidebar_canvas.yview_scroll(int(-1*(event.delta/120)), "units")

        def _sidebar_enter(event):
            self.sidebar_canvas.bind_all("<MouseWheel>", _on_sidebar_mousewheel)

        def _sidebar_leave(event):
            self.sidebar_canvas.unbind_all("<MouseWheel>")

        # sidebar_canvas ve içindeki tüm widget'lara Enter/Leave bağla
        for widget in [self.sidebar_canvas, self.sidebar_inner, self.sidebar]:
            widget.bind("<Enter>", _sidebar_enter)
            widget.bind("<Leave>", _sidebar_leave)
    ...
    def _build_record_frame(self):
        ...
        self.fps_var = ctk.StringVar(value="30"); fps_menu = ctk.CTkOptionMenu(sc, variable=self.fps_var, values=["10", "15", "24", "30", "60"], width=180); fps_menu.pack(padx=16, pady=4, anchor="w")
        ...
        cam_idx_menu = ctk.CTkOptionMenu(wc, variable=self.cam_idx_var, values=["0", "1", "2"], width=180)
        cam_idx_menu.pack(padx=16, pady=4, anchor="w")

        # --- Webcam Çözünürlük Seçimi (dinamik sorgu) ---
        ctk.CTkLabel(wc, text="Webcam Çözünürlüğü", font=ctk.CTkFont("Arial", 12), text_color=TXT2).pack(padx=16, anchor="w")
        self.cam_res_var = ctk.StringVar(value="1280×720")
        self._cam_res_menu = ctk.CTkOptionMenu(
            wc, variable=self.cam_res_var,
            values=["1280×720"],   # başlangıçta varsayılan; sorgu butonuyla güncellenir
            width=180
        )
        self._cam_res_menu.pack(padx=16, pady=2, anchor="w")
        ctk.CTkButton(
            wc, text="🔍  Desteklenen Çözünürlükleri Bul",
            height=28, fg_color=CARD2, hover_color=BORDER,
            font=ctk.CTkFont("Arial", 11),
            command=self._query_cam_resolutions
        ...
        self.pip_pos_var = ctk.StringVar(value="bottom-right"); ctk.CTkOptionMenu(wc, variable=self.pip_pos_var, values=["bottom-right", "bottom-left", "top-right", "top-left"], width=180).pack(padx=16, pady=(2, 4), anchor="w")
        ...
        self.rate_var = ctk.StringVar(value="44100"); ctk.CTkOptionMenu(ac, variable=self.rate_var, values=["22050", "44100", "48000"], width=180).pack(padx=16, pady=4, anchor="w")
    ...
    def _build_help_frame(self):
        ...
        ctk.CTkLabel(ac, text="📺 YOUTUBE ADRESİM: https://www.youtube.com/@yazılımhocası-p9c", font=ctk.CTkFont("Arial", 14), text_color=TXT2).pack(anchor="w", padx=16, pady=(2,10))
        ctk.CTkLabel(ac, text="💻 GİTHUB ADRESİM: https://github.com/emralkac21", font=ctk.CTkFont("Arial", 14), text_color=TXT2).pack(anchor="w", padx=16, pady=(2,10))
        ctk.CTkLabel(ac, text="🎬 Kullanma Kılavuzu: https://youtu.be/epveUk2KvoQ?si=ku3ZG0Dq7fPwb4es", font=ctk.CTkFont("Arial", 14), text_color=TXT2).pack(anchor="w", padx=16, pady=(2,10))
        return f
    ...
    def _apply_engine_settings(self):
        ...
        # Webcam çözünürlüğü: "1280×720" → (1280, 720)
        try:
            res_str = self.cam_res_var.get().replace("×", "x")
            rw, rh = res_str.split("x")
            self.engine.webcam_res = (int(rw), int(rh))
        except Exception:
            self.engine.webcam_res = (1280, 720)
    ...
    def _start_recording(self):
        ...
        if self.engine.audio_on and AUDIO_OK:
            self.btn_mic_mute.configure(state="normal", text="🎙️ Mikrofon Açık", fg_color=ACCENT2)
    ...
    def _stop_recording(self):
        ...
        self.btn_mic_mute.configure(state="disabled", text="🎙️ Mikrofon Açık", fg_color=ACCENT2)
    ...
    def _query_cam_resolutions(self):

        cam_idx = int(self.cam_idx_var.get())
        self._cam_res_menu.configure(values=["⏳ Sorgulanıyor..."], state="disabled")
        self.cam_res_var.set("⏳ Sorgulanıyor...")

        def _run():
            resolutions = query_webcam_resolutions(cam_idx)
            labels = [f"{w}×{h}" for w, h in resolutions]
            # UI güncellemelerini ana thread'de yap
            self.after(0, lambda: self._update_cam_res_menu(labels))

        threading.Thread(target=_run, daemon=True).start()

    def _update_cam_res_menu(self, labels: list):
        """Çözünürlük menüsünü yeni etiketlerle günceller."""
        if not labels:
            labels = ["1280×720"]
        self._cam_res_menu.configure(values=labels, state="normal")
        # Mevcut seçim destekleniyor mu? Yoksa en yüksek çözünürlüğü seç
        if self.cam_res_var.get() not in labels:
            self.cam_res_var.set(labels[0])
    ...
    def _pick_wm_image(self):
        ...
        if p:
            try:
                img = Image.open(p).resize((150, 60), Image.LANCZOS)
                self.engine.wm_image = img
                self.engine._wm_cache = None   # önbelleği geçersiz kıl
                self.lbl_wm_img.configure(text=os.path.basename(p))
            except Exception as e: messagebox.showerror("Hata", str(e))
    def _clear_wm_image(self):
        self.engine.wm_image = None
        self.engine._wm_cache = None   # önbelleği geçersiz kıl
        self.lbl_wm_img.configure(text="Seçilmedi")
